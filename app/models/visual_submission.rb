class VisualSubmission < ActiveRecord::Base
  ...
  def self.not_voted(juror)
     ...
     where("jury_#{juror}_vote IS NULL OR acceptance_status != ? OR acceptance_status != ?", "Denied").order("RAND()")
   end
   
  def self.voted(juror)
    where("jury_#{juror}_vote IS NOT NULL OR acceptance_status != ? OR acceptance_status != ?", "Denied").order("RAND()")
  end
